...
import com.example.demo.Dto.CustomerAllDataForPdf;
...
import com.example.demo.Dto.PdfGenerator;
...
//*******************************************  Repo Query
	...
	public List<CustomerRequestDto> insertBulkCustomerToDataBase(MultipartFile file) {
		...
		List<CustomerRequestDto> fetchedCustomersFromUserExcel = GettingAllDataOfCustomerFromExcel
				.getTheDataOfCustomerFromExcel(file);

		List<CustomerRequestDto> checkCustomerGmailWithInExcel = this
				.checkEmailExistingTwiceFromExcel(fetchedCustomersFromUserExcel);
		...
		List<Customer> bulkCustomerToSave = finalCustomersToAddToDb.stream()
				.map(user -> this.customerRequestDtoToCustomer(user)).collect(Collectors.toList());
	...
	// Pagination and Sorting and Searching for Customers data which is shwoing to
	// Admin Page.....
	@Override
	public PaginationResponse paginationAndSortingOfCustomer(PaginationReuestDto paginationReuestDto) {
		Sort sorting = paginationReuestDto.getSortedType().equalsIgnoreCase("asc")
				? Sort.by(paginationReuestDto.getSortByFieldName()).ascending()
				: Sort.by(paginationReuestDto.getSortByFieldName()).descending();
		...
		if (paginationReuestDto.getSearchByFieldName() != null) {
			pageResult = this.customerRepo
					.findByFirstNameIgnoreCaseContaining(paginationReuestDto.getSearchByFieldName(), pageable);
		} else {
			pageResult = this.customerRepo.findAll(pageable);
		}
		List<CustomerDataRowGrids> dataList = pageResult.getContent().stream().map(this::paginationDataConversion)
	...
	private CustomerDataRowGrids paginationDataConversion(Customer customer) {
		...
		customerDataRowGrids.setExpiryDate(customer.getExpiryDate());
		customerDataRowGrids.setCustomerStatus(customer.getCustomerStatus());
		customerDataRowGrids.setCustomerType(customer.getCustomerType());
	...
	public String deleteCustomerByCustomerID(int customerId) {
		try {
			customerRepo.updateCustomerStatus(customerId, ConstantFile.STATUS_INACTIVE);
			return ConstantFile.SUCCESS;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String updateCustomerStatusActive(int customerId) {
		try {
			customerRepo.updateCustomerStatus(customerId, ConstantFile.STATUS_ACTIVE);
			return ConstantFile.SUCCESS;
		} catch (Exception e) {
			return ConstantFile.ERROR_MESS;
		}
	...
	public String editCustomerDetails(CustomerDataRowGrids customerDataRowGrids) {
		try {
			Customer customer = this.customerDataRowGridsToCustomerForEditCustomer(customerDataRowGrids);
	...
	private Customer customerDataRowGridsToCustomerForEditCustomer(CustomerDataRowGrids customerDataRowGrids) {
		Customer customer = customerRepo.findByCustomerId(customerDataRowGrids.getCustomerId());
		Address address = new Address();
		customer.setFirstName(customerDataRowGrids.getFirstName());
		customer.setLastName(customerDataRowGrids.getLastName());
		customer.setContactNo(customerDataRowGrids.getContactNo());
		customer.setEmailId(customerDataRowGrids.getEmailId());
		customer.setRegistrationDate(customerDataRowGrids.getRegistrationDate());
		customer.setExpiryDate(customerDataRowGrids.getExpiryDate());
		customer.setCustomerStatus(customerDataRowGrids.getCustomerStatus());
		customer.setCustomerType(customerDataRowGrids.getCustomerType());
		customer.setGender(customerDataRowGrids.getGender());
		address.setAddressLine1(customerDataRowGrids.getAddressLine1());
		address.setAdressLine2(customerDataRowGrids.getAdressLine2());
		address.setCity(customerDataRowGrids.getCity());
		address.setPincode(customerDataRowGrids.getPincode());
		address.setState(customerDataRowGrids.getState());
		customer.setAddress(address);
